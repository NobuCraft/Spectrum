...
"""
СПЕКТР v3.0 ULTIMATE - ИСПРАВЛЕННАЯ ВЕРСИЯ (ПОЛНЫЙ ФУНКЦИОНАЛ)
"""
...
class GroqAI:
    def __init__(self, api_key: str):
        ...
        self.system_prompt = """ТЫ — СПЕКТР, УМНЫЙ И ДРУЖЕЛЮБНЫЙ ПОМОЩНИК.
ТЫ ОТВЕЧАЕШЬ КОРОТКО, ЯСНО И ПО ДЕЛУ.
НЕ ИСПОЛЬЗУЙ МАТ, НЕ ОСКОРБЛЯЙ ПОЛЬЗОВАТЕЛЕЙ."""
...
class Database:
    ...
    def get_banlist(self) -> List[Dict]:
        ...
        self.cursor.execute("SELECT id, first_name, username, ban_reason, ban_date FROM users WHERE banned = 1")
    ...
    def get_chat_bookmarks(self, chat_id: int) -> List[Dict]:
        """Получить публичные закладки чата"""
        self.cursor.execute("""
    ...
    def get_pending_timers(self) -> List[Dict]:
        """Получить все ожидающие таймеры"""
        now = datetime.now().isoformat()
        self.cursor.execute("""
            SELECT * FROM timers
    ...
    def get_user_grids(self, user_id: int) -> List[Dict]:
        """Получить сетки пользователя"""
        self.cursor.execute("SELECT * FROM chat_grids WHERE owner_id = ?", (user_id,))
        return [dict(row) for row in self.cursor.fetchall()]
    ...
    def has_invisible_bonus(self, user_id: int) -> bool:
        """Проверить наличие бонуса невидимости"""
        self.cursor.execute("""
            SELECT * FROM user_bonuses 
    ...
    def is_invisible_banned(self, chat_id: int, user_id: int) -> bool:
        """Проверить, забанен ли пользователь в невидимке"""
    ...
    def vote_for_ban(self, vote_id: int, user_id: int, vote: bool) -> bool:
        ...
        voters = json.loads(vote_data[10])
        ...
        if vote:
            new_for = vote_data[8] + 1
            new_against = vote_data[9]
        else:
            new_for = vote_data[8]
            new_against = vote_data[9] + 1
    ...
    def is_vip(self, user_id: int) -> bool:
        """Проверить, активен ли VIP статус"""
    ...
    def is_premium(self, user_id: int) -> bool:
        """Проверить, активен ли PREMIUM статус"""
    ...
    def add_boss_kill(self, user_id: int):
        """Увеличить счетчик убийств боссов"""
...
Я — **{BOT_NAME} v{BOT_VERSION}**, многофункциональный чат-менеджер.

📋 **Основные возможности:**
• Модерация чата (муты, баны, варны)
• Экономика (монеты, неоны, глитчи)
• Игры (мафия, дуэли, боссы)
• Ачивки и профили
• Кружки по интересам
• И многое другое!
...
🔍 **Выберите действие ниже:**
    ...
    async def cmd_help(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        text = (
            ...
            f"{s.cmd('+Модер @user', 'назначить модератором')}\n"
...
/farm — ферма глитчей
    ...
    async def cmd_ban(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        """Забанить пользователя (ИСПРАВЛЕНО)"""
        user = update.effective_user
        user_data = self.db.get_user(user.id)
        text = update.message.text
        chat_id = update.effective_chat.id
    ...
    async def cmd_setup_info(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if isinstance(update, Update) and update.callback_query:
            await update.callback_query.edit_message_text(text, parse_mode=ParseMode.MARKDOWN, disable_web_page_preview=True)
        else:
...
3️⃣ 📄 Бумага
    ...
    async def cmd_top_glitches(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        for i, row in enumerate(top, 1):
            ...
            medal = "🥇" if i == 1 else "🥈" if i == 2 else "🥉" if i == 3 else f"{i}."
            text += f"{medal} {name} — {row[2]} 🖥\n"
        await update.message.reply_text(text, parseMode=ParseMode.MARKDOWN)
...
/buybonus [название] [срок] — купить
    ...
    async def handle_edited_message(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        """Обработка редактированных сообщений - ИСПРАВЛЕНО: просто игнорируем, чтобы не было ошибок"""
        pass
    ...
    async def handle_left_member(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        """Прощание с покинувшими участниками"""
    ...
    async def error_handler(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        """Обработка ошибок - ИСПРАВЛЕНО: не спамит при редактировании"""
        error = context.error
        
        # Игнорируем ошибки, связанные с редактированием сообщений
    ...
    def setup_handlers(self):
        ...
        # ВАЖНО: Обработчик для редактированных сообщений - просто игнорируем, чтобы не было ошибок
        self.app.add_handler(MessageHandler(filters.UpdateType.EDITED_MESSAGE, self.handle_edited_message))
