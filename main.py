...
class SimpleAI:
    def __init__(self):
        print("✅ SimpleAI инициализирован")
    
    async def get_response(self, message: str) -> str:
        msg_lower = message.lower().strip()
        
        # Приветствия
        if any(word in msg_lower for word in ["привет", "здравствуй", "хай", "ку", "hello"]):
        ...
        # Как дела
        elif any(word in msg_lower for word in ["как дела", "как ты", "что делаешь"]):
            return random.choice([
        ...
        # Что ты умеешь
        elif any(word in msg_lower for word in ["что ты умеешь", "твои функции", "что можешь"]):
            return "📋 Мои возможности: /bosses, /shop, /profile, /daily, /help"
        
        # Боссы
        elif any(word in msg_lower for word in ["босс", "битва", "сражение"]):
            return "👾 Боссы ждут тебя! Используй /bosses"
        
        # Профиль
        elif any(word in msg_lower for word in ["профиль", "статистика", "кто я"]):
            return "📊 Твой профиль в /profile"
        
        # Магазин
        elif any(word in msg_lower for word in ["магазин", "купить", "шоп"]):
            return "🛍 Магазин: /shop"
        
        # Награда
        elif any(word in msg_lower for word in ["награда", "бонус", "дейли"]):
            return "🎁 Ежедневная награда: /daily"
        
        # Помощь
        elif any(word in msg_lower for word in ["помощь", "хелп", "команды"]):
