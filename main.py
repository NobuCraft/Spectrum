...
class GeminiAI:
    ...
    async def get_response(self, user_id: int, message: str) -> str:
        try:
            ...
            # Системный промпт для официального стиля
            system_prompt = (
...
class IrisKeyboard:
    """Класс для создания красивых клавиатур в стиле Iris"""
...
class GameBot:
    def __init__(self):
        ...
        self.f = IrisFormatter()  # Форматтер
        self.keyboard = IrisKeyboard()  # Клавиатуры
        self.setup_handlers()
    ...
    async def cmd_my_stats(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        # Активность по часам (имитация)
        hours = list(range(24))
        activity = [random.randint(0, 10) for _ in hours]
    ...
    async def button_callback(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if data == "noop":
            ...
            command_map = {
                ...
                "edit_motto": "edit_motto",
                "edit_gender": "edit_gender"
            }
            await getattr(self, f"cmd_{command_map[data]}")(update, context)
